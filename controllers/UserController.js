...
exports.getAllUsers = async (req, res, next) => {
  try {
    ...
    let users = await UsersModel.find({}).select('email userName avatar isAdmin');
...
exports.getUser = async (req, res, next) => {
   const id = req.params.id
    //get all users from users collection
    let user = await UsersModel.findById(id).select('email userName avatar isAdmin');
    res.json(user);
    // console.log(user);
};
...
exports.loginUser= async(req,res,next)=>{
  ...
  if (!user) {
    res.json({ error: "NO such user found in DB. Email or password is invalid" });
