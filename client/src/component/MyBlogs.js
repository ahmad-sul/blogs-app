...
export default function MyBlogs() {
    ...
    console.log('delete user blog');
        const userId =localStorage.getItem('userId')
        const userBlogs= await axios.delete(baseURL+'blogs/deleteBlog/'+userId)
    ...
    return user? (
     ...
     {blog && blog.length >0 ? (blog.map((el)=>{
         return ( <div key={el.id}>
             <li className='d-flex m-3 justify-content-between border'>
                ...
                <button type="button" className="btn btn-outline-danger me-2" onClick={(el)=>deleteBlog(el)}>Delete</button>
                </div>
